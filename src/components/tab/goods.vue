  ...
  <div>
    <div class="goods">
    	<div class="menu-wrapper" ref="menuWrapper">
    		<ul>
    			<li v-for="(item,index) in goods" :key="index" class="menu-item" ref="menuList" :class="{'current':currentIndex===index}" @click="selectMenu(index,$event)">
      		...
      		<li v-for="(item,index) in goods" :key="index" class="food-list" ref="foodList">
      			...
      			<ul>
      				<li @click="selectFood(food,$event)" v-for="(food,index) in item.foods" :key="index" class="food-item">
      					<div class="icon">
      						<img width="57" height="57" :src="food.icon" alt="image">
      					</div>
      					<div class="content">
                  ...
                  <div class="cartcontrol-wrapper">
                    ...
                    <!-- 小球动画，子组件cartcontrol.vue派发事件cart-add，父组件中监听并添加监听函数addFood -->
                  </div>
      					</div>
      				</li>
      			</ul>
...
export default {
  ...
  methods: {
    ...
    addFood(target) {//监听子元素的cart-add事件，执行addFood函数,
      this._drop(target);
    },
    _drop(target) {//通过监听的函数获得目标组件shopcart.vue的DOM对象：this.$refs.shopcart.drop(target).在shopcart.vue中编写drop()获取位置
